...
template<typename T>
void Heuristic_Bracket(std::vector<std::vector<Eigen::MatrixXf>>& Jacobian_products,
	std::vector<std::vector<Tensor<T>>>& Hessian_products,
	std::vector<Triplet<T>>& HB) {

	for (auto itr : HB) {
		Jacobian_products[itr.x][itr.z] = Jacobian_products[itr.x][itr.y] * Jacobian_products[itr.y - 1][itr.z];
		Hessian_products[itr.x][itr.z] = MatXTens(Jacobian_products[itr.x][itr.y], Hessian_products[itr.y - 1][itr.z]) +
			TensXMatCross(TensXMat(Hessian_products[itr.x][itr.y], Jacobian_products[itr.y - 1][itr.z]),
				Jacobian_products[itr.y - 1][itr.z]);
	}
}
...
int main(int argc, char* argv[]) {
	...
	assert(argc == 4); std::ifstream in_1(argv[1]); std::ifstream in_2(argv[2]); std::ifstream in_3(argv[3]);
	...
	auto duration_4 = std::chrono::duration_cast<std::chrono::microseconds>(t6 - t5).count();
	
	// Heuristic Bracketing approach
	auto t7 = std::chrono::high_resolution_clock::now();
	Heuristic_Bracket(Jacobian_products, Hessian_products, HB);
	auto t8 = std::chrono::high_resolution_clock::now();
	auto duration_3 = std::chrono::duration_cast<std::chrono::microseconds>(t8 - t7).count();
	...
	std::cout << "heuristic bracketing: " << duration_3 << std::endl;
	std::cout << "optimized bracketing: " << duration_4 << std::endl;
